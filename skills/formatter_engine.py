...
def _set_table_cell_margins(
    table, top_cm=0.0, bottom_cm=0.0, left_cm=0.05, right_cm=0.05
):
    tbl = table._tbl
    tbl_pr = tbl.tblPr
    if tbl_pr is None:
...
def _is_table_title(text):
    ...
    if len(text) > 30:
        return False
    return (
        re.match(r"^表\s*(?:\d+|[一二三四五六七八九十]+)(?:[-—._、]\d+)?", text)
        is not None
    )
...
def detect_para_type(text, index, total, alignment, all_texts, all_texts_index=None):
    ...
    if index >= total - 10 and len(text) < 30:
        if re.search(
            r"(公司|局|委|部|厅|院|所|中心|办公室|集团|银行|学校|大学|医院)$", text
        ):
            return "signature"
        remaining_texts = (
            all_texts[all_texts_index + 1 :] if all_texts_index is not None else []
        )
...
def format_paragraph(para, fmt, para_type, line_spacing_pt=28, first_line_bold=False):
    ...
    pf.alignment = align_map.get(
        fmt.get("align", "justify"), WD_ALIGN_PARAGRAPH.JUSTIFY
    )
    pf.left_indent = Pt(0)
    ...
    if first_line_bold and para_type == "body":
        ...
        for run in para.runs:
            set_font(
                run, fmt["font_cn"], fmt["font_en"], fmt["size"], fmt.get("bold", False)
            )
...
def format_document(input_path, output_path, preset_name="official"):
    ...
    for idx, block in enumerate(blocks):
        ...
        prev_para_is_title = isinstance(prev_block, Paragraph) and _is_table_title(
            prev_block.text
        )
        prev_para_is_unit = isinstance(prev_block, Paragraph) and _is_table_unit(
            prev_block.text
        )
        ...
        for row_idx, row in enumerate(table.rows):
            ...
            for col_idx, cell in enumerate(row.cells):
                if table_cfg.get("optimize", True):
                    _set_cell_borders(
                        cell, size_pt=table_cfg.get("border_size_pt", 0.5)
                    )
        ...
        if table_cfg.get("after_table_blank_line", True):
            ...
            if unit_para is not None:
                ...
                if not (
                    isinstance(after_unit, Paragraph) and not after_unit.text.strip()
                ):
                    _insert_paragraph_after_paragraph(unit_para, text="")
            else:
                if not (
                    isinstance(next_block, Paragraph) and not next_block.text.strip()
                ):
